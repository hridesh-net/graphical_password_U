...
from main import MSE_THRESHOLD
...
def create_user(username, password_image_path, num_segments):
    ...
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute('INSERT INTO users VALUES (?, ?)', (username, segments_data_str))
    conn.commit()
    conn.close()
...
def authenticate_user(username, password_image_path, num_segments):
    ...
    # stored_segments = [np.fromstring(stored_segment_data, dtype=np.uint8).reshape((height, segment_width, 3)) for stored_segment_data in stored_segments_data]
    stored_segments = [np.frombuffer(bytes(stored_segment_data.encode()), dtype=np.uint8).reshape((height, segment_width, 3)) for stored_segment_data in stored_segments_data]
...
class GraphicalPasswordGUI:
    ...
    def authenticate(self):
        ...
        if not self.password_image_path:
            tk.messagebox.showerror('Error', 'Please select a password image')
            return
        ...
        if authenticated:
            tk.messagebox.showinfo('Success', 'Authentication successful')
        else:
